export default class ReplayOverlay {
  ...
  showIntro() {
    ...
    if (this._hint)    this._hint.textContent  = '[space] or → to begin'
  ...
  showStep(stepNum, total, explanation, isLast) {
    ...
    if (this._hint)    this._hint.textContent    = isLast ? '[space] to finish' : '[space] next hop'
  ...
  onNext(fn) {
    if (this._btn) this._btn.onclick = fn
  }
}
